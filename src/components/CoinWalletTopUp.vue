...
const { addCoinToWallet } = useMyCoinWallet()
const myCoin = useMyCoinWallet()
</script>
...
<template>
  ...
  >
    ...
    <div class="flex flex-wrap gap-4 justify-center">
      <button
        v-for="(numberOfCoins, coinsName) in coin"
        :key="coinsName"
        @click="addCoinToWallet(coinsName as CoinKey, numberOfCoins)"
      ...
      >
        <span class="font-bold text-2xl mb-1">+{{ numberOfCoins }}</span>
        <span
    ...
    <div class="mt-8 bg-white rounded-2xl p-6 shadow-inner border-2 border-blue-100">
      ...
      <ul class="mb-4">
        <li
          v-for="(amount, coinsName) in myCoin.numOfCoinWallet"
          :key="coinsName"
          class="flex justify-between py-1 border-b last:border-b-0 text-base"
        >
          <span class="capitalize font-medium">{{ coinsName }}</span>
